1. Editado SP = SP_Auth_Login.sql

2. El token hay que guardarlo en algun lado para validarlo en el otro SP que hace el cambio de contraseña

3. SP_Colaborador_GetById hace el select con *

4. Faltan los triggers que actualizan los campos de fecha actualizacion en las tablas 

5. SP_Colaborador_List hace el select con *

6. SP_Colaborador_Create No regresa el ID del colaborador
...
11. SP_Bitacora_LogError no guarda el nombre del SP como tal sino algo similar puede deribar a dificultad para encontrar errores 
	con la funcion COALESCE(ERROR_PROCEDURE(), '') detecta el SP que fallo
12. SP_Bitacora_LogError no guarda que fallo en el SP solo pone ERROR_MESSAGE o sea que no lo toma como funcion sino como texto 
...
16. Editado SP_Permiso_Solicitar el insert y update se llaman igual cambie el update a SP_Permiso_Solicitar_update

17. SP_Permiso_List hace el select con *

18. Editado SP_Reporte_AsistenciaDiaria agregue la fecha
